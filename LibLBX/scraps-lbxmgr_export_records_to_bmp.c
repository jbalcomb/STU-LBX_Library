...
int lbxmgr_export_records_to_bmp(LBX_DATA * lbx)
{
    ...
    {
        printf("%s (%d of %d) [%s]\n", lbx->meta->file_name, itr_entry_index + 1, lbx->header->entry_count, lbx->header->string_table->entry[itr_entry_index].name);
        // display_lbx_image_header(lbx, itr_entry_index);
        // display_lbx_image_custom_palette_header(lbx, itr_entry_index);

        // ats_build_file_path_and_name(&export_file_path, lbx->file->export_directory_path_bin, lbx->meta->records->entry[itr_entry_index].record_file_name_base, export_file_extension_bin);
        ats_build_file_path_and_name(&export_file_path, lbxmgr_data->export_directory_path_bmp, lbx->meta->records->entry[itr_entry_index].record_file_name_base, export_file_extension_bmp);
        ...
        if (LBXMGR_DEBUG_VERBOSE_MODE) printf("DEBUG: Record Type: (%d) %s\n", lbx->meta->records->entry[itr_entry_index].type, l_LBX_Record_Type_Description[lbx->meta->records->entry[itr_entry_index].type]);

        if (lbx->meta->records->entry[itr_entry_index].type == LBX_RECORD_TYPE_IMAGE)
        {
            fseek(lbx->file_stream, lbx->header->offset_table->entry[itr_entry_index].begin, SEEK_SET);

            char * record_data;
            record_data = (char *)malloc(sizeof(char) * (lbx->meta->records->entry[itr_entry_index].size + 1));
            fread(record_data, lbx->meta->records->entry[itr_entry_index].size, 1, lbx->file_stream);

            /* TODO(JWB): maybe, figure out how to just have pointers to record_data for the key locations - header and data starting positions */
            ...
            if (lbx_image_header->frame_count == 1)
            {
                ...
                */
                ...
                if (LBXMGR_DEBUG_STRUGGLE_MODE) printf("DEBUG: BEGIN: image_data_decoded_buffer = (char *)malloc(lbx_image_header->width * lbx_image_header->height);\n");
                char * image_data_decoded_buffer;
                image_data_decoded_buffer = (char *)malloc(lbx_image_header->width * lbx_image_header->height);
                if (image_data_decoded_buffer == nullptr)
                {
                    printf("FAILURE: image_data_decoded_buffer = (char *)malloc(%d);\n", lbx_image_header->width * lbx_image_header->height);
                    exit(EXIT_FAILURE);
                }
                if (LBXMGR_DEBUG_STRUGGLE_MODE) printf("DEBUG: END: image_data_decoded_buffer = (char *)malloc(lbx_image_header->width * lbx_image_header->height);\n");



                if (LBXMGR_DEBUG_STRUGGLE_MODE) printf("DEBUG: BEGIN: memset(image_data_decoded_buffer, 211, lbx_image_header->width * lbx_image_header->height);\n");
                memset(image_data_decoded_buffer, 211, lbx_image_header->width * lbx_image_header->height);
                if (LBXMGR_DEBUG_STRUGGLE_MODE) printf("DEBUG: END: memset(image_data_decoded_buffer, 211, lbx_image_header->width * lbx_image_header->height);\n");
