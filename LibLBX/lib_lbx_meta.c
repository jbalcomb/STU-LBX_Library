...
{
    if (LBX_DEBUG_MODE) printf("DEBUG: BEGIN: populate_all_lbx_meta_data()\n");
    ...
    if (LBX_DEBUG_MODE) printf("DEBUG: END: populate_all_lbx_meta_data()\n");
}
...
void populate_lbx_file_meta_data(LBX_DATA * lbx)
{
     ...
     */
    lbx_file_path_and_file_name(lbx);
    lbx_file_name_base(lbx);

    /*
     * File Size
     */
    lbx->meta->file_size = lbx_file_size_via_seek(lbx->file_stream);
     ...
     */
    lbx_earliest_timestamp_via_stat(lbx);
     ...
     * Checksums - CRC32
     */
    lbx_compute_crc32_file(lbx->file_stream, &lbx->meta->file_checksum_crc32);
...
void populate_lbx_record_meta_data(LBX_DATA * lbx)
{
    ...
    char l_LBX_Record_Type_Description[12][34] = {
            "EMPTY",
            "Array",
            "XMIDI",
            "VOC",
