...
char * get_record_data(LBX_DATA * lbx, unsigned int entry_number)
{
    ...
    fseek(lbx->file->file_stream, lbx->header->offset_table->entry[entry_number].begin, SEEK_SET);

    fread(record_data, 1, tmp_record_size, lbx->file->file_stream);
...
unsigned long int get_record_checksum_crc32(LBX_DATA * lbx, unsigned int entry_number)
{
    ...
    liblbx_compute_crc32_char_buffer(tmp_record_data, tmp_record_size, &record_checksum_crc32);
...
/* TODO(JWB): extract to utility function, the turning into a string, of a number, with padding */
char * get_record_file_name_base(LBX_DATA * lbx, unsigned int entry_number)
{
    ...
    */
    ...
    record_file_name_base = malloc(sizeof(char) * (strlen(lbx->file->file_name_base) + tmp_len_entry_number_padding_string + tmp_len_entry_number_string + 1));
    strcpy(record_file_name_base, lbx->file->file_name_base);
    ...
    */
    ...
    tmp_entry_number_string = liblbx_convert_entry_index_to_string(entry_number, LBX_ENTRY_INDEX_STRING_LENGTH_DEFAULT);
    if (LBX_DEBUG_STRUGGLE_MODE) printf("DEBUG: tmp_entry_number_string: %s\n", tmp_entry_number_string);


    if (LBX_DEBUG_STRUGGLE_MODE) printf("DEBUG: BEGIN: record_file_name_base = malloc(sizeof(char) * (strlen(lbx->file->file_name_base) + LBX_ENTRY_INDEX_STRING_LENGTH_DEFAULT + 1));\n");
    record_file_name_base = malloc(sizeof(char) * (strlen(lbx->file->file_name_base) + LBX_ENTRY_INDEX_STRING_LENGTH_DEFAULT + 1));
    if (record_file_name_base == NULL)
    {
        printf("FAILURE: record_file_name_base = malloc(sizeof(char) * (strlen(lbx->file->file_name_base) + LBX_ENTRY_INDEX_STRING_LENGTH_DEFAULT + 1));\n", sizeof(char) * (strlen(lbx->file->file_name_base) + LBX_ENTRY_INDEX_STRING_LENGTH_DEFAULT + 1));
        exit(EXIT_FAILURE);
    }
    if (LBX_DEBUG_STRUGGLE_MODE) printf("DEBUG: END: record_file_name_base = malloc(sizeof(char) * (strlen(lbx->file->file_name_base) + LBX_ENTRY_INDEX_STRING_LENGTH_DEFAULT + 1));\n");
